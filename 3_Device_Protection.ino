void backflowControl(){                                                //PV BACKFLOW CONTROL (INPUT MOSFET) 
  ...
  //   if(vin>vout+voltageDropout){bypassEnable=1;}     //CHARGER MODE: Normal Condition - Turn on Backflow MOSFET (on by default when not in MPPT charger mode)
...
void Device_Protection(){
  ...
  if(output_Mode==0){                                                                               //PSU MODE specific protection protocol
    ...
    if(vin<voltageBatteryMax+voltageDropout){IUV=1;ERR++;errorCount++;}else{IUV=0;}        //IUV - INPUT UNDERVOLTAGE: Input voltage is below battery voltage (for psu mode only)                     
  }
